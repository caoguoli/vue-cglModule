...
export default {
  ...
  methods:{
    ...
    stateView(){
      ...
      const color = this.cirObj.color;
      const title = this.cirObj.title || '';
      let canvasObj = fun.getCanvas(min, max, selWindstate, selWindstate, disable, disableCir,color,title);
    ...
    funData:function(){
      ...
      if (Math.abs(Math.abs(move_R - pan_R) - pan_Width) < 15) { // 在圆盘上滑动
        ...
        let canvasObj = fun.getCanvas(min, max, move_num, move_num, disable, false,color,title);
    ...
    canvasEndFn(){
      ...
      this.$emit('cglend',this.newCanvasWidth)
    }
  }
}
</script>
